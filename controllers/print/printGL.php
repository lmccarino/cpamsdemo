...
foreach ($details as $row) {  
    ...
    $pdf->Write(0, date('F d, Y', $gldetails->getGLDate($row)), '', 0, 'L', true, 0, false, false, 0);
    ...
    $pdf->CustomFooterText = $gldetails->getUser($row['userID'])." @ ".date('m/d/Y H:i:s', $gldetails->getGLDate($row) );

    $raffile = $row['rafNum'];    
    $stamp = getdate();
    $filestamp =  $stamp['year'].$stamp['mon'].$stamp['mday'].$stamp['hours'].$stamp['minutes'].$stamp['seconds'];
    $filename = $raffile.'_'.$filestamp.'.pdf';
...
$pdf->Output('gl.pdf', 'I');
// $pdf->Output(__DIR__ . '/pdfuploads/'.$filename, 'I');
