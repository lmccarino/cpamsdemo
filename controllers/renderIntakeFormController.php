...
$data .='<table><tr><td align="center"><u>'.$row['sworker'].'</u><br/>Social Worker</td><td align="center"><u>'.$row['requestor']."</u><br/>Requestor<br/>".$row['relation']."</td></tr></table>";
 $pdf->setHtmlVSpace($tagvs);
	...
	if (stristr(PHP_OS, 'WIN')) {
		...
		// Removed By: Teddy C. 09/14/2023 09:10.
		// $pdf->Output($fullpath,'F');
		// $command ="CALL saveintakeform($idassistdetails, '$fname',$userid, '$tk')";
	...
	// Author: Teddy C. 09/14/2023 09:10.
	...
	$result = Utility::uploadToServer($tempfileUri, 'intake', 'generatedpdf');
	...
	if($result['success'] == true){
		...
		$command ="CALL saveintakeform($idassistdetails, '$generatedfilename',$userid, '$tk')";
		$row = getrow($command);
	}
	// End Teddy C.	
