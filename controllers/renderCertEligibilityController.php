...
$age = $today->diff($bdate);
	...
	$Date=date_create($raf['dateApproved']);
...
$data .='<p>Attested by:</p>';
 $pdf->setHtmlVSpace($tagvs);
	...
	$sigfullpath2 = '../signatures/'.$sigimage2;
				...
				$x = $pdf->GetX();
				$x = $x + 26;
				$pdf->SetXY($x, $y+10); $pdf->Image('@'.$imgdata,'','',75,0,'PNG','','',true, 300); imagedestroy($im); $pdf->SetXY(0, $y+50); $pdf->SetFont ('times', '', 11);
				$pdf->Ln(2);
				$y = $pdf->GetY();
				...
				$pdf->Image($sigfullpath2, '', '', 70, 0, $ext, '', '', true, 300);
				$x = $pdf->GetX();
				...
				// TO
				// $data2 .='<td align="center"><u><strong>'.$signatory2['fullname'].'</strong></u><br/>Asst. City Administrator (Operations)</td></tr></table>';
				// End Changes
	...
	if (stristr(PHP_OS, 'WIN')) {
		...
		// Removed By: Teddy C. 09/14/2023 09:07.
		// $pdf->Output($fullpath,'I');
		// $command ="CALL savecerteligibility($idassistdetails, '$fname',$userid, '$tk')";
	...
	// Author: Teddy C. 09/14/2023 09:07.
	...
	$result = Utility::uploadToServer($tempfileUri, 'eligibility', 'generatedpdf');
	...
	if($result['success'] == true){
		...
		$command ="CALL savecerteligibility($idassistdetails, '$generatedfilename',$userid, '$tk')";
		$row = getrow($command);
	}
	// End Teddy C.
